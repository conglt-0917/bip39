(function () {
  ...
  function p2wpkhInP2shSelected() {
    return (
      bip49TabSelected() || (bip141TabSelected() && DOM.bip141semantics.val() == 'p2wpkh-p2sh')
    );
  }
      ...
      setTimeout(function () {
        ...
        if (hasPrivkey) {
          ...
          if (useBip38) {
            if (isGRS())
              ...
              privkey = libs.bip38.encrypt(keyPair.d.toBuffer(), false, bip38password, function (
                p
              ) {
                console.log('Progressed ' + p.percent.toFixed(1) + '% for index ' + index);
              });
        ...
        if (isSegwit) {
          ...
          if (isP2wpkh) {
            ...
            var scriptPubKey = libs.bitcoin.script.witnessScriptHash.output.encode(
            ...
            var redeemScript = libs.bitcoin.script.witnessScriptHash.output.encode(
              libs.bitcoin.crypto.sha256(witnessScript)
            );
  ...
  function stripHexPrefix(address) {
    ...
    var hasPrefix = address.substring(0, 2) === '0x' || address.substring(0, 2) === '0X';
